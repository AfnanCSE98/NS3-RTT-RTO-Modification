...
void
RttMeanDeviation::FloatingPointUpdatePeakHopper (Time m)
{
  NS_LOG_FUNCTION (this << m);
 ...
 //gain complement
  ...
  double gErr = err.ToDouble (Time::S) * c_gain;
  m_estimatedRtt += Time::FromDouble (gErr, Time::S);
...
//double prev_alpha = 1/8;
//double prev_beta = 1/4;
Time pr(0.1);
    ...
    { 
      ...
      // must convert Time to floating point and back again
        ...
        if (rttShift && variationShift)
          {
            //NS_LOG_UNCOND("int update");
            IntegerUpdate (m, rttShift, variationShift);
          }
        else
          {
            //NS_LOG_UNCOND("float update");
            FloatingPointUpdatePeakHopper(m);
          }
    }
  else
    { // First sample
...
/*
void 
RttMeanDeviation::Measurement_PH (Time m , double gain)
{
  NS_LOG_FUNCTION (this << m);
  if (m_nSamples)
    { 
